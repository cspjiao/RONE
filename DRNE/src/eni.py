...
class eni(object):
    def __init__(self, graph, args, sess):
        ...
        self.save_path = os.path.join(self.args.save_path,
                                      '{}_{}_{}_{}'.format(self.args.save_suffix,
                                                           self.args.embedding_size,
                                                           self.args.alpha, self.args.lamb))
    ...
    def build_model(self):
        with tf.variable_scope('Placeholder'):
            ...
            self.neighborhood_placeholder = tf.placeholder(tf.int32,
                                                           (None, self.args.sampling_size),
                                                           name='neighborhood_placeholder')
            self.label_placeholder = tf.placeholder(tf.float32, (None,),
                                                    name='label_placeholder')

        self.data = network.next_batch(self.graph, self.degree_max, sampling=True,
        ...
        with tf.variable_scope('Embeddings'):
            self.embeddings = tf.get_variable('embeddings',
                                              ...
                                              initializer=tf.constant_initializer(
                                                  utils.init_embedding(self.degree,
                                                                       self.degree_max,
                                                                       self.args.embedding_size)))
        ...
        with tf.variable_scope('LSTM'):
            cell = tf.contrib.rnn.DropoutWrapper(
                ...
                tf.contrib.rnn.LayerNormBasicLSTMCell(num_units=self.args.embedding_size,
                                                      layer_norm=False),
        ...
        with tf.variable_scope('Guilded'):
            self.predict_info = tf.squeeze(
                tf.layers.dense(self.lstm_output, units=1, activation=utils.selu))

        with tf.variable_scope('Loss'):
            self.structure_loss = tf.losses.mean_squared_error(
                tf.nn.embedding_lookup(self.embeddings, self.nodes_placeholder),
                self.lstm_output)
            ...
            self.total_loss = self.structure_loss + self.args.alpha * self.orth_loss + \
                              self.args.lamb * self.guilded_loss
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs_to_train):
            ...
            for i in range(total_num):
                ...
                _, total_loss, structure_loss, orth_loss, guilded_loss = self.sess.run(
                    [self.train_op, self.total_loss, self.structure_loss, self.orth_loss,
                     self.guilded_loss], feed_dict=batch_data)
                n += 1
                end = time.time()
                # process = psutil.Process(os.getpid())
                print((
                    "epoch: {}/{}, batch: {}/{}, loss: {:.6f}, structure_loss: {:.6f}, "
                    "orth_loss: {:.6f}, guilded_loss: {:.6f}, time: {:.4f}s").format(
                    epoch, self.args.epochs_to_train, n - 1, total_num, total_loss,
                    structure_loss, orth_loss, guilded_loss, end - begin))
                cost += end - begin
                if num % 5 == 0:
            ...
            if epoch % 10 == 0:
                self.save()
        print("Embedding time: {}".format(cost))
