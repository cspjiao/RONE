...
class Factorization:
    ...
    def __init__(self, args, user_size, feature_size):
        ...
        self.target = tf.compat.v1.placeholder(tf.int64, shape=[None, None])
        ...
        self.embedding_node = tf.Variable(tf.random.uniform([self.user_size, self.args.dimensions],
                                            -0.1/self.args.dimensions, 0.1/self.args.dimensions))

        self.embedding_feature = tf.Variable(tf.random.uniform([self.feature_size, self.args.dimensions],
                                            -0.1/self.args.dimensions, 0.1/self.args.dimensions))
    ...
    def __call__(self):
        ...
        return tf.reduce_mean(tf.compat.v1.losses.log_loss(self.target,self.embedding_predictions))
