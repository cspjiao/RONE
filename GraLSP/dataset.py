...
class dataset:
    def __init__(self, dataset_name="cora", purpose="link_prediction", linkpred_ratio=0.1,
        ...
        self.edge_list = []
        with open('../dataset/clf/{}.edge'.format(self.dataset_name), 'r') as infile:
            for line in infile:
        ...
        for ws in range(self.num_nodes):
            for _ in range(self.num_paths * self.num_skips):
                ...
                self.types_and_nodes[ws].append([self.node_anonym_walktypes[ws][wk].astype(int), random.choice(
                    self.node_walks[ws][wk][:self.node_walk_radius[ws][wk]])])
        self.types_and_nodes = np.array(self.types_and_nodes).astype(int)

        # self.node_features = np.load("data/" + dataset_name + "/features.npy")
        self.node_features = np.eye(self.num_nodes)
        self.feature_dim = len(self.node_features[0])

        if self.purpose == "classification":
            self.node2label = np.zeros((self.num_nodes))

            with open("../dataset/clf/{}.lbl".format(self.dataset_name)) as infile:
                for line in infile:
                    elems = line.rstrip().split(" ")
    ...
    def generate_batch(self, batch_size, window_size):
        ...
        return np.array(keys).astype(int), np.array(labels).astype(int), np.array(negs).astype(int), np.array(
            walk_key).astype(int), np.array(walk_label).astype(int), np.array(walk_neg).astype(int)
    ...
    def sample_negative_links(self, negative_num):
        ...
        while cnt < negative_num:
            ...
            if (s, d) in self.edge_list or (s, d) in self.pred_edge_list or (d, s) in self.edge_list or (
                    d, s) in self.pred_edge_list:
                continue
