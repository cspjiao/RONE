...
def glrd_sparse(V, G, F, r, err_V, err_F):
    ...
    for k in range(r):
        ...
        constraints_F = [x_F >= 0]
        constraints_F += [cvx.sum(x_F) <= err_F]
...
def task(args):
    if args.dataset[0:3] == "clf":
        ...
        embed = pd.read_csv("../embed/GLRD-S/"+args.dataset+'.emb', encoding='utf-8', sep=',')
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(node_feature_file, encoding='utf-8', sep=',', header=None)
    x = [0]
    actual_fx_matrix = df.drop(df.columns[x],axis=1,inplace=True).values
    print(actual_fx_matrix)'''
    ...
    minimum_description_length = 1e60
    ...
    embedding.to_csv("../embed/GLRD-S/" + args.dataset +".emb", index = None)
    task(args)
