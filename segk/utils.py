...
def write_to_file(path, nodes, embeddings, delimiter=' '):
	...
	out_embedding = pd.DataFrame(
		np.concatenate([np.array(nodes).reshape(-1, 1).tolist(), embeddings], axis=1),
		columns=columns)
	out_embedding = out_embedding.sort_values(by=['id'])
	out_embedding.to_csv(path, index=None)
...
def extract_egonets(edgelist, radius, node_labels=None):
	...
	for i in range(1, radius + 1):
		for node in nodes:
			leaves = [v for v in egonet_node_labels[node] if
					  egonet_node_labels[node][v] == (i - 1)]
			for leaf in leaves:
...
def load_graph_classification_data(ds_name, use_node_labels):
	...
	with open("datasets/graph_classification/%s/%s_graph_indicator.txt" % (
			ds_name, ds_name), "r") as f:
		c = 1
		for line in f:
	...
	with open("datasets/graph_classification/%s/%s_A.txt" % (ds_name, ds_name),
			  "r") as f:
		for line in f:
			edge = line[:-1].split(",")
	...
	if use_node_labels:
		node_labels = dict()
		with open("datasets/graph_classification/%s/%s_node_labels.txt" % (
				ds_name, ds_name), "r") as f:
			c = 1
			for line in f:
	...
	class_labels = list()
	with open("datasets/graph_classification/%s/%s_graph_labels.txt" % (ds_name,
																		ds_name),
			  "r") as f:
		for line in f:
